...
const superuserRoutes: FastifyPluginAsync = async (fastify): Promise<void> => {
  ...
  fastify.get('/api/superuser/stores', { preHandler }, async (_request, reply) => {
    try {
      ...
      fastify.log.error('Error fetching stores for superuser', error)
      return reply.status(500).send({ success: false, message: 'Failed to fetch stores', error: error.message })
    }
  })

  // POST /api/superuser/stores/:storeId/toggleStatus
  fastify.post('/api/superuser/stores/:storeId/toggleStatus', { preHandler }, async (request, reply) => {
    try {
      ...
      return reply.send({ success: true, message: `Store ${storeId} disabled status set to ${disabled}` })
    } catch (error: any) {
      return reply.status(500).send({ success: false, message: 'Error updating store status', error: error.message })
    }
  })
}
