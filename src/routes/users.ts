...
const usersRoutes: FastifyPluginAsync = async (fastify): Promise<void> => {
  ...
  fastify.post("/api/users/profile", authenticate, async (request, reply) => {
    try {
      const uid = request.user?.uid;
      const { phone, displayName } = request.body as {
        phone?: string;
        displayName?: string;
      };
        ...
        if (userData) {
          return reply.send({
            ...
            user: {
              ...
              email:
                request.user?.role === "superuser" ? userData.email : undefined,
            },
          });
        } else {
          return reply
      ...
      try {
        ...
        for (const [roomId, room] of Object.entries(
          ...
          const history = room.history
            ? (Object.values(room.history) as any[])
            : [];
          const myItems = history.filter(
            (item: any) => item.claimed_by === uid,
          );

          if (myItems.length > 0) {
            const totalSpent = myItems.reduce(
            ...
            result.push({
              ...
              items: myItems.sort(
                (a: any, b: any) => (b.archivedAt || 0) - (a.archivedAt || 0),
              ),
              totalSpent,
            });
          }
        ...
        result.sort((a, b) => {
          const latestA = Math.max(
            ...a.items.map((i: any) => i.archivedAt || 0),
          );
          const latestB = Math.max(
            ...b.items.map((i: any) => i.archivedAt || 0),
          );
