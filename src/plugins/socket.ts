...
const socketPlugin: FastifyPluginAsync = async (fastify) => {
  ...
  io.on('connection', (socket: AuthenticatedSocket) => {
    fastify.log.info(`Client connected: ${socket.id} (User: ${socket.user?.uid || 'Guest'})`)
    ...
    socket.on('placeBid', async (data: { roomId: string; productId: string; bidAmount: number }) => {
      if (!socket.user) {
        return socket.emit('error', { message: 'Unauthorized. Must be logged in to bid.' })
      }
      ...
      try {
        const productRef = fastify.db.ref(`rooms/${roomId}/products/${productId}`)
        ...
        if (bidAmount <= currentHighest) {
          return socket.emit('error', { message: 'Bid must be higher than current highest bid.' })
        }
